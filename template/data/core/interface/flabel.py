...
#The size of a FLabel is equal to the scale / 100. BGECore Design must guarentee that all fonts have 4 vertex and medians of size 1.
class FLabel(Widget):
	"""
	The basic Text object of BGECore.
	
	FLabels are dynamic texts that can be instantiated at runtime in any given position, used in Widgets or as a replacemnet for BGE text objects. To replace
	a text object the TextObject must have two game properties "*(String)* Font" and "*(String[Right,Center,Left])* Align". FLabels need a font object to work. Font objects are created as
